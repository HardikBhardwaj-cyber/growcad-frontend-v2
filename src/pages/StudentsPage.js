...
function BatchBadge({ name }) {
  if (!name || name === '-') return <span style={{ color: 'rgba(255,255,255,0.30)', fontSize: 12 }}>—</span>;
  return (
    <span
...
export default function StudentsPage() {
  ...
  const handleEdit = (s) => {
    setEditing(s);
    setForm({
      name: s.name, phoneNumber: s.phoneNumber || '',
      parentPhoneNumber: s.parentPhoneNumber || '',
      email: s.email || '', batchId: s.batchId || '',
  ...
  if (loading) {
    return (
      <div data-testid="students-page" className="animate-fade-in">
        <div className="flex items-center justify-center h-48">
          <div className="flex flex-col items-center gap-3">
            <div className="w-8 h-8 rounded-full animate-spin"
              style={{ border: '2px solid rgba(108,60,244,0.22)', borderTopColor: '#7c4ff5' }} />
            <p className="text-[11px] uppercase tracking-widest" style={{ color: 'rgba(255,255,255,0.30)' }}>Loading…</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div data-testid="students-page" className="animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          <h1 className="text-[1.35rem] font-bold tracking-tight" style={{ color: 'rgba(255,255,255,0.95)' }}>
            Students
          </h1>
      ...
      <div className="flex flex-col sm:flex-row gap-2.5 mb-5">
        ...
        <div className="relative">
          <select
            ...
            style={{ ...INPUT, width: 'auto', minWidth: 160, paddingRight: 32, appearance: 'none', cursor: 'pointer' }}
          ...
          <table className="w-full dark-table" data-testid="student-table">
            <thead>
              <tr>
                ...
                <th className="text-left" style={{ width: 100 }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {students.map(s => (
                    ...
                    <div className="flex items-center gap-0.5">
                      ...
                      {isAdmin && (
                        <>
                          ...
                          >
                            <Send size={13}
                              className={reminding[s.id] === true ? 'animate-pulse' : ''} />
                          </button>
                  ...
                  <td colSpan={5} style={{ padding: '52px 16px', textAlign: 'center', border: 'none' }}>
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-[11px]" style={{ color: 'rgba(255,255,255,0.28)' }}>
                        {search || batchFilter ? 'Try adjusting your filters' : 'Add your first student to get started'}
                      </p>
                    </div>
                  </td>
                </tr>
